...
if not display_df.empty:
    ...
    with col1:
        ...
        for _, row in display_df.iterrows():
            ...
            folium.Marker(
                location=[row["lat"], row["lon"]],
                icon=folium.DivIcon(html=f'<div style="font-size: 14px; font-weight: bold; color: black; text-align: center;">veh-{row["vehicle_id"]}</div>')
            ).add_to(m)
            
            folium.Marker(
                location=[row["lat"], row["lon"]],
                icon=folium.DivIcon(html=f'<div style="font-size: 12px; font-weight: bold; color: black; background-color: white; padding: 6px; border-radius: 6px;"> {f"stop-{row['Stop Sequence']}"}</div>')
            ).add_to(m)
